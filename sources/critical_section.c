...
static void	display_action(int philo_id, const char *action, t_table *table)
{
...
static void	eat(t_philo *philo)
{
	...
	philo->last_meal = get_time_ms(philo->table);
	philo->meal_count++;
...
void	sim_start_delay(time_t start_time, t_table *table)
{
...
void	*critical_section(void *arg)
{
	...
	philo = (t_philo *)arg;
	if (philo->id % 2 == 0)
		ft_usleep(philo->table->meal_time/2, philo->table);
